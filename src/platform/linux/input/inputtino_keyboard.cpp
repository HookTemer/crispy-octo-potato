...
namespace platf::keyboard {
  ...
  static const std::map<short, short> key_mappings = {
    ...
    {KEY_LEFTSHIFT, 0x10}, {KEY_LEFTCTRL, 0x11},{KEY_CAPSLOCK, 0x14},
    {KEY_ESC, 0x1B},       {KEY_SPACE, 0x20},   {KEY_PAGEUP, 0x21},
    {KEY_PAGEDOWN, 0x22},  {KEY_END, 0x23},     {KEY_HOME, 0x24},
    {KEY_LEFT, 0x25},      {KEY_UP, 0x26},      {KEY_RIGHT, 0x27},
    {KEY_DOWN, 0x28},      {KEY_SYSRQ, 0x2C},   {KEY_INSERT, 0x2D},
    {KEY_DELETE, 0x2E},
    ...
    {KEY_RIGHTSHIFT, 0xA1},{KEY_RIGHTCTRL, 0xA3},
    {KEY_LEFTALT, 0xA4},   {KEY_RIGHTALT, 0xA5},
    {KEY_SEMICOLON, 0xBA}, {KEY_EQUAL, 0xBB},  {KEY_COMMA, 0xBC},
    {KEY_MINUS, 0xBD},     {KEY_DOT, 0xBE},    {KEY_SLASH, 0xBF},
    {KEY_GRAVE, 0xC0},     {KEY_LEFTBRACE, 0xDB},{KEY_BACKSLASH, 0xDC},
    {KEY_RIGHTBRACE, 0xDD},{KEY_APOSTROPHE, 0xDE},{KEY_102ND, 0xE2},
  };
  ...
  void update(input_raw_t *raw, uint16_t modcode, bool release, uint8_t flags) {
    ...
    if (raw->display) {
      if (modcode >= keysyms.size()) return;
      const auto ks = keysyms[modcode].keysym;
      if (ks == UNKNOWN) return;
      const auto keycode = x11::KeysymToKeycode(raw->display, ks);
      if (keycode == 0) return;
      x11::tst::FakeKeyEvent(raw->display, keycode, !release, CurrentTime);
  ...
  void unicode(input_raw_t *raw, char *utf8, int size) {
    ...
    // Note: unicode input via XTest not supported (requires CTRL+SHIFT+U method with uinput)
  }
}  // namespace platf::keyboard
